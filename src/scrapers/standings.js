...
async function scrapeStandings(page, leagueId) {
    ...
    try {
        ...
        const standingsData = await page.evaluate(() => {
            ...
            const teamRows = Array.from(container.children);
            ...
            for (let i = 0; i < teamRows.length; i++) {
                try {
                    ...
                    const dataRowIndex = Array.from(container.querySelectorAll('td')).indexOf(row);
                    ...
                    const dataRowCells = Array.from(document.querySelectorAll(`.ultimate-table > section > div > table > tr:nth-child(${dataRowIndex + 1}) > td`));
